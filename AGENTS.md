...
- **Invite-only registration (closed rollout):** the backend gates `POST /v1/auth/register` behind invite codes held server-side and never shipped in the app. The client collects the code in Settings → Synchronisation (stored in `SyncDataStore`, not the Keystore credential store) and sends it on first registration. Codes are distributed out of band; how they are configured and validated is documented in the folicular repository, not here.
